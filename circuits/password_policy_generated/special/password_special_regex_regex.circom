...
// regex: ^P[A-Za-z0-9!@#$%^&*]*[!@#$%^&*][A-Za-z0-9!@#$%^&*]*;$
template PasswordSpecialRegexRegex(maxHaystackBytes, maxMatchBytes) {
    ...
    for (var i = 0; i < maxMatchBytes; i++) {
        ...
        // Transition 9: 2 -[33]-> 3
        isValidTransition[9][i] <== CheckByteTransition()(2, 3, 33, currStates[i], nextStates[i], haystack[i]);
        // Transition 10: 2 -[35-38]-> 3
        isValidTransition[10][i] <== CheckByteRangeTransition()(2, 3, 35, 38, currStates[i], nextStates[i], haystack[i]);
        // Transition 11: 2 -[42]-> 3
        isValidTransition[11][i] <== CheckByteTransition()(2, 3, 42, currStates[i], nextStates[i], haystack[i]);
        // Transition 12: 2 -[64]-> 3
        isValidTransition[12][i] <== CheckByteTransition()(2, 3, 64, currStates[i], nextStates[i], haystack[i]);
        // Transition 13: 2 -[94]-> 3
        isValidTransition[13][i] <== CheckByteTransition()(2, 3, 94, currStates[i], nextStates[i], haystack[i]);
        // Transition 14: 3 -[33]-> 3
        isValidTransition[14][i] <== CheckByteTransition()(3, 3, 33, currStates[i], nextStates[i], haystack[i]);
        // Transition 15: 3 -[35-38]-> 3
        isValidTransition[15][i] <== CheckByteRangeTransition()(3, 3, 35, 38, currStates[i], nextStates[i], haystack[i]);
        // Transition 16: 3 -[42]-> 3
        isValidTransition[16][i] <== CheckByteTransition()(3, 3, 42, currStates[i], nextStates[i], haystack[i]);
        // Transition 17: 3 -[48-57]-> 3
        isValidTransition[17][i] <== CheckByteRangeTransition()(3, 3, 48, 57, currStates[i], nextStates[i], haystack[i]);
        // Transition 18: 3 -[64-90]-> 3
        isValidTransition[18][i] <== CheckByteRangeTransition()(3, 3, 64, 90, currStates[i], nextStates[i], haystack[i]);
        // Transition 19: 3 -[94]-> 3
        isValidTransition[19][i] <== CheckByteTransition()(3, 3, 94, currStates[i], nextStates[i], haystack[i]);
        // Transition 20: 3 -[97-122]-> 3
        isValidTransition[20][i] <== CheckByteRangeTransition()(3, 3, 97, 122, currStates[i], nextStates[i], haystack[i]);
        // Transition 21: 3 -[59]-> 4
        isValidTransition[21][i] <== CheckByteTransition()(3, 4, 59, currStates[i], nextStates[i], haystack[i]);
